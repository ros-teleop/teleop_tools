...
class MouseTeleop():
    def __init__(self):
        ...
        # Create window:
        self._root = six.moves.tkinter.Tk()
        ...
        # Create canvas:
        self._canvas = six.moves.tkinter.Canvas(self._root, bg='white')

        # Create canvas objects:
        self._canvas.create_arc(0, 0, 0, 0, fill='red', outline='red',
                width=1, style=six.moves.tkinter.PIESLICE, start=90.0, tag='w')
        ...
        self._text_v_x = six.moves.tkinter.StringVar()
        if self._holonomic:
            self._text_v_y = six.moves.tkinter.StringVar()
        self._text_w   = six.moves.tkinter.StringVar()

        self._label_v_x = six.moves.tkinter.Label(self._root,
                anchor=six.moves.tkinter.W, textvariable=self._text_v_x)
        if self._holonomic:
            self._label_v_y = six.moves.tkinter.Label(self._root,
                    anchor=six.moves.tkinter.W, textvariable=self._text_v_y)
        self._label_w = six.moves.tkinter.Label(self._root,
                anchor=six.moves.tkinter.W, textvariable=self._text_w)
